  ...
  const fetchPullRequests = async () => {
    ...
    try {
      const res = await fetch(
        `/api/github/pull-requests?projectId=${selectedProjectId}&state=${stateFilter}`
      );
      const data = await res.json();
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{t("pullRequestsSubtitle")}</p>
      </div>
              ...
              <div className="flex gap-4 items-end">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    options={[
                      { value: "open", label: t("openPRs") },
                      { value: "closed", label: t("closedPRs") },
                      { value: "all", label: t("allPRs") },
                    ]}
                  />
                </div>
                <Button
        ...
        <Card className="border-destructive">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              {errorType === "no-repo" && (
                <p className="text-sm">{t("missingRepo")}</p>
              )}
              {errorType === "rate-limit" && (
                <p className="text-sm">{t("rateLimitExceeded")}</p>
              )}
              {errorType === "not-found" && (
                <p className="text-sm">{t("repoNotFound")}</p>
              )}
              {errorType === "api-error" && (
            ...
            <div className="space-y-2">
              {filteredPRs.map((pr) => (
                <Card
                  ...
                  onClick={() =>
                    router.push(
                      `/github/pull-requests/${pr.number}?projectId=${selectedProjectId}`
                    )
                  }
                >
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={
                              pr.state === "open" ? "success" : "secondary"
                            }
                          >
                            {pr.state}
                          </Badge>
                        ...
                        <div className="flex items-center gap-3 mt-2 flex-wrap">
                          ...
                          {pr.changedFiles > 0 && (
                            <span className="text-xs text-muted-foreground">
                              {pr.changedFiles} {t("changedFiles")}
                            </span>
                          )}
                          {pr.additions > 0 && (
